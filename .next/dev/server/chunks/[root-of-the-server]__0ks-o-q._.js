...
"[project]/src/lib/intel/narrator.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function buildDailyBrief(snapshot, assets, pools, previousSnapshot) {
    const bullets = [];
    let tvlChangePct = 0;
    if (previousSnapshot && previousSnapshot.totalTvl > 0) {
        tvlChangePct = (snapshot.totalTvl - previousSnapshot.totalTvl) / previousSnapshot.totalTvl * 100;
    }
    bullets.push(`Total ecosystem TVL is ${fmtUsd(snapshot.totalTvl)} (${fmtPct(tvlChangePct)} vs previous snapshot).`);
    bullets.push(`24h volume is ${fmtUsd(snapshot.totalVolume)} across ${snapshot.assetCount} assets and ${snapshot.poolCount} pools.`);
    ...
    if (assets.length > 0) {
        ...
        bullets.push(`Healthiest asset: ${topAsset.symbol} (health ${topAsset.healthScore.toFixed(1)}, liquidity ${topAsset.liquidityScore.toFixed(1)}).`);
    }
    if (pools.length > 0) {
        ...
        if (sustainable.length > 0) {
            ...
            bullets.push(`Top sustainable yield pool: ${topPool.assetSymbol} at ${topPool.apr.toFixed(2)}% APR with ${fmtUsd(topPool.tvlUsd)} TVL.`);
        }
    }
    if (snapshot.marketConcentration > 0.35) {
        bullets.push(`Market concentration is elevated (HHI ${snapshot.marketConcentration.toFixed(2)}) — ecosystem is top-heavy.`);
    }
    ...
    return {
        ...
        evidence: [
            ...
            {
                metric: "tvl_change_pct",
                value: round(tvlChangePct, 3)
            },
            {
                metric: "concentration",
                value: snapshot.marketConcentration
            }
        ]
    };
}
function buildEcosystemHealthSummary(snapshot, assets, pools) {
    const bullets = [];
    const bullish = [];
    const bearish = [];
    ...
    const sustainablePools = pools.filter((p)=>p.apr >= 10 && p.apr <= 80 && p.healthScore >= 60);
    if (sustainablePools.length > 0) {
        bullish.push(`${sustainablePools.length} pools show sustainable yield (10–80% APR) with strong health.`);
    }
    const riskyAssets = assets.filter((a)=>a.healthScore < 40);
    if (riskyAssets.length > 0) {
        bearish.push(`${riskyAssets.length} assets have a health score below 40: ${riskyAssets.slice(0, 5).map((a)=>a.symbol).join(", ")}.`);
    }
    if (snapshot.marketConcentration > 0.3) {
        bearish.push(`Market concentration HHI = ${snapshot.marketConcentration.toFixed(2)} — a single asset drives the majority of TVL.`);
    }
    bullets.push(...bullish.map((b)=>`Bullish: ${b}`));
    bullets.push(...bearish.map((b)=>`Bearish: ${b}`));
    const body = `Bullish signals: ${bullish.length}. Bearish signals: ${bearish.length}. ` + `Concentration HHI is ${snapshot.marketConcentration.toFixed(2)} (${snapshot.marketConcentration > 0.3 ? "high" : "moderate"}). ` + `${healthyAssets.length}/${assets.length} assets are healthy. ` + `${sustainablePools.length}/${pools.length} pools offer sustainable yield.`;
    return {
        ...
        evidence: [
            ...
            {
                metric: "healthy_assets",
                value: healthyAssets.length
            },
            {
                metric: "risky_assets",
                value: riskyAssets.length
            },
            {
                metric: "sustainable_pools",
                value: sustainablePools.length
            }
        ]
    };
}
function buildOpportunitySummary(opps) {
    const bullets = [];
    ...
    for (const o of hidden){
        const tvl = o.metrics.tvl ?? 0;
        const fees = o.metrics.fees_24h ?? 0;
        bullets.push(`Gem: ${o.title} — TVL ${fmtUsd(tvl)}, fees ${fmtUsd(fees)}/day, score ${o.score.toFixed(1)}.`);
    }
    ...
    if (bestYield.length > 0) {
        const apr = bestYield[0].metrics.apr ?? 0;
    ...
    return {
        ...
        evidence: [
            ...
            {
                metric: "best_yield_count",
                value: bestYield.length
            },
            {
                metric: "hidden_gem_count",
                value: hidden.length
            },
            {
                metric: "risk_count",
...
const QUESTION_PATTERNS = [
    [
        /\b(today|happen|brief|daily)\b/i,
        "daily_brief"
    ],
    [
        /\b(health|healthy|bullish|bearish)\b/i,
        "ecosystem_health"
    ],
    [
        /\b(opportunit|yield|gem|best)\b/i,
        "opportunity"
    ],
    [
        /\b(liquidity|inflow|outflow|capital)\b/i,
        "liquidity"
    ],
    [
        /\b(pool|apr|fees)\b/i,
    ...
    [
        /\b(asset|stock|momentum|growth|winner)\b/i,
        "assets"
    ],
    [
        /\b(risk|warn|danger)\b/i,
        "risk"
    ],
    [
        /\b(watch|week|ahead)\b/i,
        "watch"
    ]
];
function answerLiquidity(snapshot) {
    const inflows = snapshot.largestInflows.slice(0, 3);
    const outflows = snapshot.largestOutflows.slice(0, 3);
    const parts = [
        `Total TVL is ${fmtUsd(snapshot.totalTvl)}.`
    ];
    if (inflows.length > 0) {
        parts.push("Largest inflows: " + inflows.map((i)=>`${i.symbol} (+${fmtUsd(i.usd)})`).join(", "));
    }
    if (outflows.length > 0) {
        parts.push("Largest outflows: " + outflows.map((o)=>`${o.symbol} (-${fmtUsd(o.usd)})`).join(", "));
    }
    return parts.join(" ");
}
function answerPools(pools) {
...
function answerAssets(assets) {
    if (assets.length === 0) return "No assets are currently indexed.";
    const topTvl = [
        ...assets
    ].sort((a, b)=>b.tvlUsd - a.tvlUsd).slice(0, 3);
    const topGrowth = [
        ...assets
    ].sort((a, b)=>b.growthScore - a.growthScore).slice(0, 3);
    const topHealth = [
        ...assets
    ].sort((a, b)=>b.healthScore - a.healthScore).slice(0, 3);
    return [
...
function answerRisk(snapshot, opps) {
    const risks = opps.filter((o)=>o.kind === "risk");
    const parts = [];
    if (risks.length > 0) {
        parts.push(`${risks.length} risk flags: ` + risks.slice(0, 3).map((o)=>`${o.title} (${o.explanation})`).join("; "));
    } else {
...
function answerQuestion(question, snapshot, assets, pools, opportunities) {
    let handler = "";
    ...
    switch(handler){
        ...
        case "watch":
            ...
            break;
        default:
            body = `I can answer questions about liquidity, pools, assets, opportunities, risks, or what to watch. ` + `Right now the ecosystem has ${fmtUsd(snapshot.totalTvl)} TVL across ${snapshot.assetCount} assets and ${snapshot.poolCount} pools.`;
    }
    return {
        kind: "ask",
        title: `Q: ${question}`,
...
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$intel$2f$narrator$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/lib/intel/narrator.ts [app-route] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$intel$2f$llm$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/lib/intel/llm.ts [app-route] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$intel$2f$storage$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/lib/intel/storage.ts [app-route] (ecmascript)");
;
;
 ...
 */ function buildContext() {
    const s = __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$intel$2f$cache$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["cache"].snapshot;
    const lines = [];
    lines.push(`ECOSYSTEM: total TVL ${fmtUsd(s.totalTvl)}, 24h volume ${fmtUsd(s.totalVolume)}, ` + `${s.assetCount} assets, ${s.poolCount} pools, concentration HHI ${s.marketConcentration.toFixed(2)}.`);
    if (s.largestInflows.length) {
        lines.push("INFLOWS: " + s.largestInflows.slice(0, 3).map((i)=>`${i.symbol} +${fmtUsd(i.usd)}`).join(", ") + ".");
    }
    if (s.largestOutflows.length) {
        lines.push("OUTFLOWS: " + s.largestOutflows.slice(0, 3).map((o)=>`${o.symbol} -${fmtUsd(o.usd)}`).join(", ") + ".");
    }
    lines.push("ASSETS (tokenized equities):");
    for (const a of __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$intel$2f$cache$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["cache"].assets.slice(0, 10)){
        lines.push(`- ${a.symbol}: price $${a.price.toFixed(2)}, premium/discount ${a.premiumDiscount.toFixed(2)}%, ` + `TVL ${fmtUsd(a.tvlUsd)}, 24h vol ${fmtUsd(a.volume24h)}, ` + `liquidity ${a.liquidityScore.toFixed(0)}/100, health ${a.healthScore.toFixed(0)}/100, growth ${a.growthScore.toFixed(0)}/100.`);
    }
    lines.push("POOLS:");
    for (const p of __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$intel$2f$cache$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["cache"].pools.slice(0, 8)){
        lines.push(`- ${p.assetSymbol}: TVL ${fmtUsd(p.tvlUsd)}, 24h vol ${fmtUsd(p.volume24h)}, ` + `fees ${fmtUsd(p.fees24h)}, APR ${p.apr.toFixed(2)}%, vol/TVL ${p.volumeToTvl.toFixed(2)}, health ${p.healthScore.toFixed(0)}/100.`);
    }
    if (__TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$intel$2f$cache$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["cache"].narratives.length) {
        lines.push("NARRATIVES (7d momentum):");
        for (const n of __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$intel$2f$cache$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["cache"].narratives){
            lines.push(`- ${n.narrative}: ${n.mentions} mentions, ${n.deltaPct}% vs prior, reach ${n.reach}, ${n.breadth} sources, stage ${n.stage}.`);
        }
    }
    if (__TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$intel$2f$cache$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["cache"].opportunities.length) {
        lines.push("OPPORTUNITIES:");
        for (const o of __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$intel$2f$cache$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["cache"].opportunities.slice(0, 6)){
...
async function POST(request) {
    try {
        ...
        const ruleBased = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$intel$2f$narrator$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["answerQuestion"])(question, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$intel$2f$cache$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["cache"].snapshot, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$intel$2f$cache$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["cache"].assets, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$intel$2f$cache$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["cache"].pools, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$intel$2f$cache$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["cache"].opportunities);
        // Free-form, data-grounded answer (null → use rule-based body).
        const llmBody = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$intel$2f$llm$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["answerWithContext"])(question, buildContext());
        const insight = llmBody ? {
            ...ruleBased,
            body: llmBody
        } : ruleBased;
        await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$intel$2f$storage$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["writeAiSummary"])(insight);
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(insight);
    } catch (e) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0ks-o-q._.js.map