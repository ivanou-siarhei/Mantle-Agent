...
function buildDigestMarkdown(args) {
    ...
    const poolBySymbol = new Map(pools.map((p)=>[
            p.assetSymbol,
            p
        ]));
    const top = [
    ...
    lines.push(`Coverage: ${snapshot.assetCount} equities · TVL ${fmtUsd(snapshot.totalTvl)} · 24h vol ${fmtUsd(snapshot.totalVolume)}`);
    lines.push("");
    ...
    for (const a of top){
        ...
        void p;
        lines.push(`| ${a.symbol} | $${a.price.toFixed(2)} | ${pct(a.premiumDiscount)} | ${fmtUsd(a.tvlUsd)} | ${fmtUsd(a.volume24h)} | ${a.healthScore.toFixed(0)}/100 |`);
    }
    lines.push("");
...
//# sourceMappingURL=%5Broot-of-the-server%5D__1jfvhk3._.js.map